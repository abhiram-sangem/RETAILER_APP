...
export default function App() {
  ...
  const customerStatementData = useMemo(() => {
    ...
    invoices.forEach(inv => {
      ...
      if (invNameNorm === targetNameNorm) {
        ...
        if (!inv.isReturn) {
          debit = amount;
          credit = isPayLater ? 0 : amount;
        } else {
          credit = amount;
          debit = isPayLater ? 0 : amount; 
        }
  ...
  const saveToDrafts = () => {
    ...
    const draftName = window.prompt(
      "Enter a name to easily identify this draft:", 
      activeTab.activeCustomer ? activeTab.activeCustomer.name : `Draft ${new Date().toLocaleTimeString()}`
    );
  ...
  function addToCart(product) {
    ...
    if (existing) {
      if (existing.quantity >= product.stock) {
        return window.alert(`Cannot add more. We only have ${product.stock} of ${product.name} in stock.`)
      }
  ...
  function updateQuantity(index, val) {
    ...
    if (quantity > item.stock) {
      return window.alert(`Cannot exceed available inventory (${item.stock} left).`)
    }
    const newCart = activeTab.cart.map((itm, idx) => idx === index ? { ...itm, quantity } : itm)
    updateActiveTab({ cart: newCart })
  }
  ...
  function submitFinalSale() {
    ...
    if (activeTab.editingInvoiceId) {
      invoiceService.update(
        activeTab.editingInvoiceId,
        activeTab.activeCustomer.name,
        activeTab.cart,
        activeBillingDetails.subtotal, 
        activeTab.discountPercent,
        activeBillingDetails.cgst,
        activeBillingDetails.sgst,
        activeBillingDetails.finalTotal,
        finalPaymentMethod,
        activeTab.saleDate
      ).then(invoice => {
  ...
  function submitPurchaseCart() {
    if (editingPurchaseInvoiceId) {
      purchaseInvoiceService.update(
        ...
        window.alert(`Purchase Invoice updated successfully!`)
        resetPurchaseState()
      ...
      purchaseInvoiceService.create(
        ...
        window.alert(`Purchase Invoice recorded successfully!`)
        resetPurchaseState()
  ...
  function handleGenerateReceipt() {
    ...
    receiptService.create(receiptCustomer.id, receiptAmount, receiptDiscount, receiptMethod, receiptDate, receiptRemarks)
      .then(() => {
        ...
        setReceiptSearch('');
        setReceiptRemarks('Payment received with thanks.');
      })
  ...
  function handleSaveProduct() {
    ...
    if (isEditMode) {
      ...
      productService.updateProduct(
        ...
        loadProducts(); loadHistory(); closeProductModal() 
      })
    } else {
  ...
  function handleDeleteProduct(id, name) {
    if (window.confirm(`Are you sure you want to completely delete "${name}"? This cannot be undone.`)) { 
      productService.deleteProduct(id).then(() => {
        loadProducts(); closeProductModal();
      }) 
    }
  }
  ...
  function handleSaveInventory() {
    ...
    productService.updateProduct(
      ...
      loadProducts(); loadHistory(); closeInventoryModal() 
    })
  }
  ...
  function handleSaveCustomer() {
    ...
    action.then(() => { 
      loadCustomers(); closeCustomerModal() 
    })
  }
