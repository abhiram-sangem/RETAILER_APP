...
import { 
  formatProductId, formatPurchaseInvoiceId, formatMoney 
} from '../utils/formatters';
import { purchaseInvoiceService } from '../services/api';
import Pagination from '../components/Pagination';

export default function PurchaseManager({ 
  view, setView, products, purchaseInvoices, purchaseInvoiceHistory, 
    ...
    <>
      ...
      {view === 'purchase-summary-screen' && (
        <div className="card">
          ...
          <div className="invoice-summary-grid margin-top-large">
            <div className="info-block"><span className="info-label">Seller Name</span><strong className="info-value">{purchaseSellerName}</strong></div>
        ...
        <div className="card">
          ...
          <div className="table-responsive">
            <table className="block-table data-table">
              <thead><tr><th>System ID</th><th>Seller Name</th><th>Vendor Bill No</th><th>Total Amount</th><th>Date</th><th>Action</th></tr></thead>
              <tbody>
          ...
          <Pagination totalItems={filteredPurchaseInvoices.length} itemsPerPage={itemsPerPage} setItemsPerPage={setItemsPerPage} currentPage={currentPage} setCurrentPage={setCurrentPage} />
        </div>
      )}
      ...
      {view === 'purchase-invoice-details' && selectedPurchaseInvoice && selectedPurchaseInvoiceMath && (
        <div className="card">
          ...
          <div className="invoice-summary-grid margin-top-large">
            <div className="info-block"><span className="info-label">Seller Name</span><strong className="info-value">{selectedPurchaseInvoice.sellerName}</strong></div>
      ...
      {view === 'purchase-edit-history' && (
        <div className="card">
          ...
          <div className="table-responsive">
            <table className="block-table data-table">
              <thead><tr><th>Edit Date</th><th>Original Purchase ID</th><th>Seller Name</th><th>Details</th></tr></thead>
              <tbody>
          ...
          <Pagination totalItems={filteredPurchaseInvoiceHistory.length} itemsPerPage={itemsPerPage} setItemsPerPage={setItemsPerPage} currentPage={currentPage} setCurrentPage={setCurrentPage} />
        </div>
      )}
        ...
        <div className="modal-overlay no-print">
          <div className="modal-content modal-large">
            <div className="card-header header-actions border-none pb-0">
              ...
              <button className="btn btn-secondary action-buttons-right" onClick={() => { setShowAddPurchaseProductModal(false); setSearchQuery(''); }}>Close</button>
            </div>
            ...
            <div className="table-responsive modal-scroll-area-nobottom">
              <table className="block-table data-table table-fixed">
                <thead><tr><th className="col-20">ID</th><th className="col-35">Product Name</th><th className="col-15">HSN Code</th><th className="col-15">Buy Price</th><th className="col-15">Stock</th></tr></thead>
                <tbody>
                  {filteredProducts.map(product => (
                    <tr key={product.id} className="product-row available" onClick={() => addToPurchaseCart(product)} title="Click block to add to purchase cart">
                  ...
                  {filteredProducts.length === 0 && <tr><td colSpan={5} className="empty-state">No products found.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
